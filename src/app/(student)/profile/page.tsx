...
export default function ProfilePage() {
  ...
  async function handleSave() {
    if (!profile || !form.full_name.trim()) { toast.error(t.cabinet.prof.nameRequired); return; }
    setSaving(true);
    ...
    toast.success(t.cabinet.prof.saved);
  }

  async function submitReview() {
    if (!reviewText.trim() || reviewText.length < 10) { toast.error(t.cabinet.prof.tooShort); return; }
    if (!profile) return;
    ...
    toast.success(t.cabinet.prof.reviewSentTitle + " " + t.cabinet.prof.reviewHint);
  }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="font-display font-bold text-3xl mb-1">{t.cabinet.prof.title}</h1>
      </motion.div>
          ...
          <div className="flex-1 min-w-0">
            {editing ? (
              <div className="space-y-3">
                <div><label className="text-xs font-medium text-muted-foreground mb-1 block">{t.cabinet.prof.fullName}</label><input value={form.full_name} onChange={e => setForm({ ...form, full_name: e.target.value })} className="input-field" /></div>
                <div><label className="text-xs font-medium text-muted-foreground mb-1 block">{t.cabinet.prof.username}</label><input value={form.username} onChange={e => setForm({ ...form, username: e.target.value })} className="input-field" placeholder="@username" /></div>
                <div><label className="text-xs font-medium text-muted-foreground mb-1 block">{t.cabinet.prof.bio}</label><textarea value={form.bio} onChange={e => setForm({ ...form, bio: e.target.value })} className="input-field min-h-[60px]" /></div>
                <div className="flex gap-2">
                  <button onClick={handleSave} disabled={saving} className="btn-primary py-2 px-4 text-sm flex items-center gap-1.5 disabled:opacity-50">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} {t.common.save}</button>
      ...
      <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-3" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        {[
          ...
          { icon: Coins, value: profile.coins, label: t.cabinet.prof.coinsLabel, color: "text-neon-yellow" },
          { icon: Flame, value: profile.streak_days, label: t.cabinet.prof.streakLabel, color: "text-neon-red" },
          { icon: Trophy, value: achievements.length, label: t.cabinet.prof.achievements, color: "text-neon-purple" },
        ].map(s => (
      ...
      <motion.div className="glass-card p-5" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
        <div className="flex items-center justify-between mb-2">
          <span className="font-semibold text-sm">{t.cabinet.dash.level}: <span className={getLevelColor(profile.level)}>{getLevelLabel(profile.level, t)}</span></span>
        ...
        <p className="text-xs text-muted-foreground mt-2">{t.cabinet.prof.memberSince}: {formatDate(profile.created_at)}</p>
      </motion.div>

      {/* Achievements */}
      {achievements.length > 0 && (
        <motion.div className="glass-card p-5" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          <h3 className="font-display font-semibold text-lg mb-3">{t.cabinet.prof.achievements}</h3>
      ...
      <motion.div className="glass-card p-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
        <h3 className="font-display font-semibold text-lg mb-2 flex items-center gap-2"><MessageSquare className="w-5 h-5 text-neon-blue" /> {t.cabinet.prof.reviewTitle}</h3>
        <p className="text-sm text-muted-foreground mb-4">{t.cabinet.prof.reviewHint}</p>
        {reviewSent ? (
          <div className="text-center py-6">
            <CheckCircle2 className="w-10 h-10 text-neon-green mx-auto mb-2" />
            <p className="font-semibold text-sm">{t.cabinet.prof.reviewSentTitle}</p>
            <p className="text-xs text-muted-foreground">{t.cabinet.prof.reviewSentHint}</p>
          </div>
        ) : (
          <div className="space-y-3">
            <div>
              <label className="text-sm font-medium mb-1.5 block">{t.cabinet.prof.ratingLabel}</label>
            ...
            <div><label className="text-sm font-medium mb-1 block">{t.cabinet.prof.commentLabel}</label>
              <textarea value={reviewText} onChange={e => setReviewText(e.target.value)} className="input-field min-h-[80px]" placeholder={t.cabinet.prof.commentPlaceholder} maxLength={500} />
            ...
            <button onClick={submitReview} disabled={reviewSending || reviewText.length < 10}
              ...
              {reviewSending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />} {t.cabinet.prof.send}
            </button>
          </div>
        )}
      </motion.div>

...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="w-full max-w-md"><div className="glass-card p-10 h-96 animate-pulse" /></div>}>
      <LoginForm />
    </Suspense>
  );
}
...
function LoginForm() {
  ...
  const [errorMsg, setErrorMsg] = useState(
    errorParam === "blocked" ? "Hisobingiz bloklangan." :
    errorParam === "auth_failed" ? "Autentifikatsiyada xatolik." : ""
  );

  const supabase = createClient();

  async function checkPlacementAndRedirect() {
    // Yangi user bo'lsa placement test ga
    ...
    <motion.div className="w-full max-w-md" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <div className="glass-card p-8 md:p-10">
        <div className="text-center mb-8">
          <h1 className="font-display font-bold text-3xl mb-2">Xush kelibsiz!</h1>
        ...
        <button onClick={handleGoogleLogin} disabled={googleLoading}
          className="w-full flex items-center justify-center gap-3 bg-surface hover:bg-surface-hover border border-border rounded-xl px-4 py-3 font-medium transition-all mb-6 disabled:opacity-50">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <Link href="/forgot-password" className="text-xs text-neon-purple hover:underline">Parolni unutdingizmi?</Link>
            </div>
            <div className="relative">
        ...
        <p className="text-center text-sm text-muted-foreground mt-6">
          Hisobingiz yo'qmi? <Link href="/register" className="text-neon-purple font-medium hover:underline">Ro'yxatdan o'tish</Link>
        </p>
      </div>
    </motion.div>
  );

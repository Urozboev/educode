  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="font-display font-bold text-3xl mb-1">{t.admin.certificates}</h1>
        <p className="text-muted-foreground text-sm">{certificates.length} ta sertifikat berilgan</p>
      </motion.div>

      <div className="relative">
        <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder={t.admin.cert.searchPh} className="input-field pl-11" />
      </div>

      <div className="glass-card overflow-hidden">
        <table className="w-full">
          <thead><tr className="border-b border-border/50 text-xs text-muted-foreground font-semibold">
            <th className="text-left px-5 py-3">{t.admin.common.student}</th>
            <th className="text-left px-5 py-3">{t.admin.common.course}</th>
            <th className="text-center px-5 py-3">{t.admin.cert.number}</th>
            <th className="text-center px-5 py-3">{t.admin.common.score}</th>
            <th className="text-center px-5 py-3">{t.admin.common.date}</th>

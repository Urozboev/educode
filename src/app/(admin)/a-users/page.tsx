...
import { cn, getInitials, getLevelLabel, getLevelColor, formatDate } from "@/lib/utils";
...
export default function AdminUsersPage() {
  ...
  async function openAssignPanel(teacherId: string) {
    ...
    const { data } = await supabase.from("teacher_students").select("student_id").eq("teacher_id", teacherId);
  ...
  const roleCounts = {
    all: users.length, student: students.length,
    teacher: teachers.length, admin: users.filter(u => u.role === "admin").length,
  };
    ...
    <div className="space-y-6">
      ...
      <AnimatePresence>
        {showAssignPanel && selectedTeacherProfile && (
          <motion.div className="glass-card p-6 border-l-4 border-l-neon-blue"
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-semibold">{selectedTeacherProfile.full_name}</h2>
                  <p className="text-xs text-neon-blue">O'qituvchi · {teacherStudents.length} ta talaba biriktirilgan</p>
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={assignAllStudents} className="text-xs px-3 py-1.5 rounded-lg bg-neon-green/10 text-neon-green hover:bg-neon-green/20 transition-all">
                  Hammasini biriktirish
                </button>
                <button onClick={() => setShowAssignPanel(false)} className="p-1.5 hover:bg-accent rounded-lg">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
            ...
            {assignLoading ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 max-h-64 overflow-y-auto">
                {students.map(s => {
                  ...
                  return (
                    <button key={s.id} onClick={() => toggleStudentAssignment(s.id)}
                      className={cn("flex items-center gap-2 p-2.5 rounded-xl text-left text-sm transition-all border",
                        isAssigned
                      ...
                      <div className={cn("w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0",
                        isAssigned ? "bg-neon-green text-white" : "bg-surface")}>
          ...
          {(["all", "student", "teacher", "admin"] as const).map(r => (
            <button key={r} onClick={() => setRoleFilter(r)}
              className={cn("px-3 py-2 rounded-lg text-xs font-medium transition-all",
                roleFilter === r ? "bg-neon-purple/10 text-neon-purple border border-neon-purple/20" : "bg-surface text-muted-foreground")}>
                ...
                <tr key={u.id} className={cn("border-b border-border/30 hover:bg-surface/30", u.is_blocked && "opacity-50")}>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div><p className="font-medium text-sm">{u.full_name}</p>
                        {u.username && <p className="text-[10px] text-muted-foreground">@{u.username}</p>}</div>
                    </div></td>
                  ...
                  <td className="px-5 py-3 text-center">
                    <span className={cn("text-xs font-medium px-2 py-0.5 rounded-full",
                      u.is_blocked ? "bg-neon-red/10 text-neon-red" : "bg-neon-green/10 text-neon-green")}>
                      {u.is_blocked ? "Bloklangan" : "Aktiv"}</span></td>
                  <td className="px-5 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button onClick={() => toggleBlock(u)}
                        className={cn("p-1.5 rounded-lg", u.is_blocked ? "hover:bg-neon-green/10 text-neon-green" : "hover:bg-neon-red/10 text-neon-red")}
